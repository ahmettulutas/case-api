...
const MakePayment = () => {
  ...
  const dummyData = React.useMemo(() => generateDummyPayment(), []);
      ...
      <form className="flex flex-col gap-2" onSubmit={handleSubmit}>
        ...
        {postResponse ? (
          <div className="flex flex-col gap-1">
            ...
            <JsonContainer
              formattedJSON={JSON.stringify(postResponse, null, 2)}
            />
          </div>
        ) : null}
        {postError ? (
          <JsonContainer formattedJSON={JSON.stringify(postError, null, 2)} />
        ) : null}
      </form>
    </div>
  );
